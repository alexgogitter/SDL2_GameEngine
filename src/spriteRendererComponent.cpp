...
SpriteRendererComponent::SpriteRendererComponent(
    ...
    : Component("SpriteRenderer2D", parentObject),
...
void SpriteRendererComponent::setDrawRect(
    float x,
    float y,
    float width,
    float height)
...
void SpriteRendererComponent::setDrawSize(const glm::vec2& sizePixels)
{
    setDrawRect({
        sizePixels.x * -0.5f,
        sizePixels.y * -0.5f,
...
Transform2D SpriteRendererComponent::buildDrawTransform() const
{
    ...
    const glm::vec2 localCentre = {
        drawRectPixels.x + drawRectPixels.z * 0.5f,
        drawRectPixels.y + drawRectPixels.w * 0.5f
    };

    const float rotation = worldTransform.getRotationZ();
    const float cosine = std::cos(rotation);
    const float sine = std::sin(rotation);
    const glm::vec2 rotatedCentre = {
        localCentre.x * cosine - localCentre.y * sine,
        localCentre.x * sine + localCentre.y * cosine
    };
