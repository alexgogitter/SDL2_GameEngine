...
struct PhysicsWorld3D::Impl
{
    ...
    explicit Impl(void* physicsHandle)
        : physics(
        ...
        )
    {
        ...
        {
            std::cerr
                << "[PhysX] Failed to create the "
                   "3D CPU dispatcher.\n";

            return;
        }

        physx::PxSceneDesc description(
        ...
        description.cpuDispatcher =
            dispatcher;

        description.filterShader =
            physx::
        ...
        scene =
            physics->createScene(description);
...
bool PhysicsWorld3D::isValid() const
{
    return
        impl != nullptr &&
        impl->valid &&
...
void* PhysicsWorld3D::getPhysicsHandle() const
{
    return impl != nullptr
        ? impl->physics
        : nullptr;
...
void* PhysicsWorld3D::getSceneHandle() const
{
    return impl != nullptr
        ? impl->scene
        : nullptr;
...
float PhysicsWorld3D::getLastSimulatedSeconds() const
{
    return lastSimulatedSeconds;
}