...
Renderer3D::Renderer3D(Window& ownerWindow)
	: window(&ownerWindow)
{
}
...
bool Renderer3D::Initialize()
{
	...
	const bool meshLoaded = meshShader->loadFromFiles(
		"res/shaders/mesh3d.vert",
		"res/shaders/mesh3d.frag"
	);
	const bool outlineLoaded = meshOutlineShader->loadFromFiles(
...
bool Renderer3D::shouldRenderLayer(int objectLayer) const
{
	return IsValidLayer(objectLayer) &&
		(activeCamera == nullptr || activeCamera->rendersLayer(objectLayer));
}
...
void Renderer3D::SubmitMesh(
	...
	int objectLayer)
{
	if (mesh.vertexArray == 0 || mesh.indexCount == 0 ||
...
void Renderer3D::SubmitSelectionOutline(
	const MeshRenderState& mesh,
	const glm::mat4& worldMatrix,
	const glm::vec4& colour,
	float shellScale,
	int objectLayer)
{
	...
	selectionOutlineCommands.push_back({
		mesh.vertexArray,
		mesh.indexCount,
		worldMatrix,
		colour,
...
{
	if (meshShader == nullptr || !meshShader->isValid() ||
	...
	if (!selectionOutlineCommands.empty() && meshOutlineShader != nullptr &&
