...
bool BuildPickRay(const ImVec2 &mousePosition, const ImVec2 &imageMinimum, const ImVec2 &imageMaximum, const Camera3D &camera, Ray3D &ray)
{
    ...
    const float viewportX = (mousePosition.x - imageMinimum.x) / width;

    const float viewportY = (mousePosition.y - imageMinimum.y) / height;
...
bool IntersectSphere(const Ray3D &ray, const glm::vec3 &centre, float radius, float &hitDistance)
{
    ...
    const float nearHit = (-b - squareRoot) * inverseDenominator;

    const float farHit = (-b + squareRoot) * inverseDenominator;
...
bool IntersectUnitCube(const Ray3D &ray, const glm::mat4 &worldMatrix, float &hitDistance)
{
    ...
    const glm::vec3 localDirection = glm::normalize(glm::vec3(inverseWorld * glm::vec4(ray.direction, 0.0f)));
...
void FindPickedObject(Object &object, const Ray3D &ray, Object *&bestObject, float &bestHitDistance)
{
    if (object.isActive()) {
        float hitDistance = 0.0f;

        const bool hasMesh = object.getComponent<MeshFilterComponent>() != nullptr;

        const bool hit = hasMesh ? IntersectUnitCube(ray, object.getWorldMatrix(), hitDistance) : IntersectSphere(ray, object.getWorldPosition(), 0.55f, hitDistance);
    ...
    for (std::size_t childIndex = 0; childIndex < object.getChildCount(); ++childIndex) {
        ...
        if (child != nullptr) {
            FindPickedObject(*child, ray, bestObject, bestHitDistance);
        }
    }
}
} // namespace

void DrawSceneViewPanel(const RenderTarget &renderTarget, EditorCamera &editorCamera, Scene &scene, EditorSelection &selection, SceneViewPanelState &state)
{
    ...
    if (renderTarget.isValid() && available.x > 0.0f && available.y > 0.0f) {
        ...
        const bool gizmoConsumedClick = DrawEditorGizmoOverlay(scene, selection, editorCamera.getCamera(), imageMinimum.x, imageMinimum.y, imageMaximum.x, imageMaximum.y, state.hovered);

        if (state.hovered && !gizmoConsumedClick && ImGui::IsMouseClicked(ImGuiMouseButton_Left)) {
            Ray3D ray;

            if (BuildPickRay(ImGui::GetIO().MousePos, imageMinimum, imageMaximum, editorCamera.getCamera(), ray)) {
                ...
                for (std::size_t rootIndex = 0; rootIndex < scene.getRootObjectCount(); ++rootIndex) {
                    ...
                    if (root != nullptr) {
                        FindPickedObject(*root, ray, pickedObject, bestHitDistance);
                    }
                }
        ...
        std::snprintf(positionText, sizeof(positionText), "Scene Camera: %.1f, %.1f, %.1f", position.x, position.y, position.z);
