...
EditorCamera::EditorCamera(float viewportWidth, float viewportHeight) : camera(viewportWidth, viewportHeight)
{
    camera.setPerspective(60.0f, 0.05f, 5000.0f);
...
void EditorCamera::focus(const glm::vec3 &worldPosition, float distance)
{
    ...
    position = worldPosition - calculateForward() * distance;

    synchroniseCamera();
}

Camera3D &EditorCamera::getCamera() { return camera; }

const glm::vec3 &EditorCamera::getPosition() const { return position; }

bool EditorCamera::isLooking() const { return looking; }
