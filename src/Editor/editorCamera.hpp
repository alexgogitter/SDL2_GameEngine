...
{
  public:
    ENGINE_API EditorCamera(float viewportWidth = 960.0f, float viewportHeight = 540.0f);

    ENGINE_API ~EditorCamera();

    ENGINE_API void update(const EventListener &input, bool viewportHovered, float deltaSeconds);

    ENGINE_API void focus(const glm::vec3 &worldPosition, float distance = 10.0f);
