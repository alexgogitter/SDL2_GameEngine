...
bool ComponentRegistry::registerType(
    const char* typeName,
    const char* displayName,
    const char* category,
    ComponentFactory factory,
    const char* const* requiredTypeNames,
...
)
{
    if (
        ...
        typeName[0] == '\0' ||
        factory == nullptr ||
        impl->find(typeName) != nullptr ||
        (
            requiredTypeCount > 0 &&
            requiredTypeNames == nullptr
        )
    )
    {
        return false;
    ...
    for (
        std::size_t index = 0;
        index < requiredTypeCount;
        ++index
    )
    {
        const char* requiredType =
            requiredTypeNames[index];

        if (
            requiredType == nullptr ||
            requiredType[0] == '\0' ||
            entry->typeName == requiredType
        )
        {
            return false;
        }
        ...
        const auto duplicate = std::find(
            entry->requiredTypeNames.begin(),
            entry->requiredTypeNames.end(),
            requiredType
        );
        ...
        entry->requiredTypeNames.emplace_back(
            requiredType
        );
    }
...
bool ComponentRegistry::contains(const char* typeName) const
{
    return impl->find(typeName) != nullptr;
}

std::size_t ComponentRegistry::getTypeCount() const
{
    return impl->entries.size();
}

const char* ComponentRegistry::getTypeNameAt(
...
const char* ComponentRegistry::getDisplayName(
...
const char* ComponentRegistry::getCategory(
    const char* typeName) const
{
    const RegistryEntry* entry = impl->find(typeName);
...
std::size_t
ComponentRegistry::getRequiredTypeCount(
    const char* typeName
) const
{
    ...
    return entry != nullptr
        ? entry->requiredTypeNames.size()
        : 0;
}

const char*
ComponentRegistry::getRequiredTypeNameAt(
    const char* typeName,
    std::size_t index
) const
{
    ...
    if (
        entry == nullptr ||
        index >= entry->requiredTypeNames.size()
    )
    {
        return "";
    }
    ...
    return
        entry->requiredTypeNames[index].c_str();
}
...
bool ComponentRegistry::removeFromObject(
    const char* typeName,
    Object& parent
) const
{
    if (
        typeName == nullptr ||
        typeName[0] == '\0' ||
        getRemovalBlocker(typeName, parent)[0] != '\0'
    )
    {
        return false;
    }
    ...
    return
        parent.remove_Component(typeName) == 0;
}

Component* ComponentRegistry::createAndAttach(
    const char* typeName,
    Object& parent,
    const ComponentCreateContext& context
) const
{
    const RegistryEntry* rootEntry =
        impl->find(typeName);
            ...
            -> Component*
    {
        ...
        creationStack.push_back(
            entry->typeName
        );

        for (
            const std::string& requiredType :
            entry->requiredTypeNames
        )
        {
            const RegistryEntry* dependency =
                impl->find(
                    requiredType.c_str()
                );
            ...
            if (
                dependency == nullptr ||
                attachEntry(dependency) == nullptr
            )
            {
    ...
    Component* result =
        attachEntry(rootEntry);

    if (result == nullptr)
    {
        for (
            auto iterator =
                newlyAttached.rbegin();
            iterator != newlyAttached.rend();
            ++iterator
        )
        {
