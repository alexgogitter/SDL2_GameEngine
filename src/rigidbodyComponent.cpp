...
RigidbodyComponent::RigidbodyComponent(
    ...
    float gravityScale,
    bool isBullet)
    : Component("Rigidbody2D", parentObject), physicsWorld(&world)
{
...
RigidbodyComponent::~RigidbodyComponent()
{
    if (physicsWorld != nullptr &&
        b2World_IsValid(physicsWorld->getWorldId()) &&
...
void RigidbodyComponent::setLinearVelocityPixels(const glm::vec2& pixelsPerSecond)
{
    ...
    b2Body_SetLinearVelocity(
        bodyId,
        PhysicsWorld2D::pixelsToMetres(pixelsPerSecond)
    );
}
...
void RigidbodyComponent::applyImpulsePixels(const glm::vec2& impulse)
{
    ...
    b2Body_ApplyLinearImpulseToCenter(
        bodyId,
        PhysicsWorld2D::pixelsToMetres(impulse),
        true
    );
}
...
void RigidbodyComponent::teleportPixels(const glm::vec2& pixelPosition, float angleRadians)
{
    ...
    b2Body_SetTransform(
        bodyId,
        PhysicsWorld2D::pixelsToMetres(pixelPosition),
        b2MakeRot(angleRadians)
    );
}
...
void RigidbodyComponent::Update(std::uint64_t)
{
    ...
    parent->transform.setPosition(
        PhysicsWorld2D::metresToPixels(b2Body_GetPosition(bodyId))
    );
