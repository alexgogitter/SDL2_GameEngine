...
namespace
{
    using SnapshotValue = std::variant<
        bool,
        int,
        float,
        glm::vec2,
    ...
    ComponentSnapshot CaptureComponent(
        Component& component
    )
    {
        ...
        for (
            std::size_t index = 0;
            index < component.getPropertyCount();
            ++index
        )
        {
...
void EditorPlaySnapshot::capture(
    const Scene& scene
)
{
    impl->objects.clear();
...
void EditorPlaySnapshot::restore(
    Scene& scene
)
{
    if (!impl->captured)
...
bool EditorPlaySnapshot::isCaptured() const
{
