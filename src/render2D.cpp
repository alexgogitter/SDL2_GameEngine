...
Renderer2D::Renderer2D(Window& ownerWindow)
    : window(&ownerWindow)
{
}
...
int Renderer2D::Renderer_Init()
{
    return initialise2DRenderer() ? 0 : 1;
}
...
void Renderer2D::Renderer_Close()
{
    destroy2DRenderer();
}
...
void Renderer2D::SubmitSolidQuad2D(
...
void Renderer2D::SubmitSprite2D(
    const Transform2D& transform,
...
void Renderer2D::SubmitOutline2D(
    const Transform2D& transform,
    const glm::vec4& colour,
    int renderLayer,
    int objectLayer)
