...
class Resource_manager
{
  public:
    ...
    ENGINE_API TextureHandle loadTexture(const char *filePath, TextureColourSpace colourSpace = TextureColourSpace::SRGB);
    ...
    ENGINE_API const char *getMeshAssetPath(MeshHandle mesh) const;

    ENGINE_API std::size_t getMeshVertexCount(MeshHandle mesh) const;

    ENGINE_API std::size_t getMeshIndexCount(MeshHandle mesh) const;

    ENGINE_API bool getMeshRenderState(MeshHandle mesh, MeshRenderState &renderState) const;
    ...
    ENGINE_API void loadLevelResources(const std::vector<std::string> &texturePaths, const std::vector<font_info> &fontRequests);
  ...
  private:
    static std::string makeTextureKey(const std::string &filePath, TextureColourSpace colourSpace);
