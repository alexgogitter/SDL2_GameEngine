...
Light2DComponent::Light2DComponent(Object *parentObject, const glm::vec3 &colour, float intensity, float radius, float height) : Component("PointLight2D", parentObject)
{
    setColour(colour);
...
std::size_t Light2DComponent::GetPropertyCount() const { return 5; }

bool Light2DComponent::GetProperty(std::size_t index, ComponentProperty &property)
{
    switch (index) {
    case 0:
        property.key = "colour";
        property.displayName = "Colour";
        property.type = ComponentPropertyType::Colour3;
...
void Light2DComponent::Draw2D(Renderer2D *renderer)
{
    ...
    renderer->SubmitLight2D(submittedLight, parent->getLayer());
}
