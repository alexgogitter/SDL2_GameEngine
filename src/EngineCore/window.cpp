...
Window::Window(int width, int height, int requestedFrameCap, int requestedImageFlags, const char *requestedTitle) : title(requestedTitle), screenWidth(width), screenHeight(height), frameCap(requestedFrameCap), imageFlags(requestedImageFlags) {}
...
{
    if (window == nullptr || SDL_SetWindowFullscreen(window, enabled ? SDL_WINDOW_FULLSCREEN_DESKTOP : 0) != 0) {
        return false;
    }
...
void Window::beginFrameOnBoundTarget(int targetWidth, int targetHeight, const glm::vec4 &clearColour)
{
...
void Window::Present() { SDL_GL_SwapWindow(window); }