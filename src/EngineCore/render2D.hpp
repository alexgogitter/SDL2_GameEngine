...
class Renderer2D
{
  public:
    ...
    ENGINE_API void SubmitSolidQuad2D(const Transform2D &transform, const glm::vec4 &colour, int renderLayer = 0, int objectLayer = 0);

    /// Submits a textured 2D material command.
    ///
    /// @param transform World position, half-size scale, and rotation in radians.
    /// @param material Resolved GPU texture objects and material properties.
    ENGINE_API void SubmitSprite2D(const Transform2D &transform, const Material2DRenderState &material, int objectLayer = 0);
    ...
    /// @param renderLayer Primary stable-sort layer; larger layers draw later.
    ENGINE_API void SubmitOutline2D(const Transform2D &transform, const glm::vec4 &colour, int renderLayer = 0, int objectLayer = 0);
    ...
    ENGINE_API void SubmitLight2D(const PointLight2D &light, int objectLayer = 0);
    ...
    void Renderer_ttf(const std::string &text, TTF_Font *font, SDL_Rect *source, SDL_Rect *destination, SDL_Color &textColour);
