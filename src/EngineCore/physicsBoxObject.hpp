...
class PhysicsBoxObject : public Object
{
  public:
    ...
    PhysicsBoxObject(const std::string &name, Resource_manager &resources, Renderer2D *renderer, PhysicsWorld2D &physicsWorld, const glm::vec2 &pixelPosition, const glm::vec2 &pixelSize, BodyType2D bodyType, const glm::vec4 &colour, bool fixedRotation = true, float density = 1.0f, float friction = 0.6f, float restitution = 0.0f) : Object(name, resources, renderer, nullptr)
    {
        ...
        rigidbody = new RigidbodyComponent(this, physicsWorld, bodyType, fixedRotation);

        collider = new ColliderComponent(this, *rigidbody, pixelSize, density, friction, restitution);
