...
)
{
    ...
    )
    {
        ...
        const ImVec2 imageMaximum(
            imageMinimum.x + available.x,
            imageMinimum.y + available.y
        );

        const ImTextureID texture =
            static_cast<ImTextureID>(
                static_cast<intptr_t>(
                    renderTarget.getColourTextureId()
                )
            );
        ...
        drawList->AddText(
            ImVec2(
                imageMinimum.x + 10.0f,
                imageMinimum.y + 10.0f
            ),
            IM_COL32(235, 240, 250, 255),
            state.fullscreen
                ? "F11 or Escape: Exit Fullscreen"
                : "F11: Fullscreen Game View"
        );
    }
    else
    {
        ...
        ImGui::TextDisabled(
            "Game render target unavailable"
        );
    }
...
bool IsPointInsideGameView(
    const GameViewPanelState& state,
    float windowX,
    float windowY
)
{
    return
        state.hovered &&
        windowX >= state.imageMinimumX &&
        windowX <= state.imageMaximumX &&
        windowY >= state.imageMinimumY &&
        windowY <= state.imageMaximumY;
}

glm::vec2 MapPointToGameView(
    const GameViewPanelState& state,
    float windowX,
    float windowY,
...
)
{
    ...
    return {
        normalisedX *
            static_cast<float>(
                std::max(renderTargetWidth, 1)
            ),

        normalisedY *
            static_cast<float>(
                std::max(renderTargetHeight, 1)
            )
    };
}