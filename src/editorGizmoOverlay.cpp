...
namespace
{
    ...
    )
    {
        ...
        projected.position.x =
            imageMinimum.x +
            (normalised.x * 0.5f + 0.5f) *
                width;

        projected.position.y =
            imageMinimum.y +
            (1.0f -
                (normalised.y * 0.5f + 0.5f)) *
                height;
    ...
    )
    {
        ...
        const ImVec2 lensPoints[4] = {
            ImVec2(centre.x + 6.0f, centre.y - 4.5f),
            ImVec2(centre.x + 6.0f, centre.y + 4.5f),
            ImVec2(centre.x + 13.0f, centre.y + 8.0f),
            ImVec2(centre.x + 13.0f, centre.y - 8.0f)};
    ...
    )
    {
        ...
        const bool showCamera =
            cameraComponent != nullptr &&
            cameraComponent->isEnabled();

        const bool showLight =
...
{
    if (
        imageMaximumX <= imageMinimumX ||
        imageMaximumY <= imageMinimumY
    )
    {
        return false;
    }
