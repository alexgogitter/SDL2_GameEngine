...
{
public:
    ENGINE_API explicit CameraComponent(
        Object* parent
    );
    ...
    ENGINE_API CameraProjection getProjectionType()
        const override;

    ENGINE_API void setViewportSize(
        float width,
        float height
    ) override;

    ENGINE_API void setProjectionType(
        CameraProjection projection
    );

    ENGINE_API void setOutputTarget(
        CameraOutputTarget output
    );

    ENGINE_API CameraOutputTarget
        getOutputTarget() const;

    ENGINE_API void setPriority(int value);
    ENGINE_API int getPriority() const;

    ENGINE_API void setClearColour(
        const glm::vec4& colour
    );

    ENGINE_API const glm::vec4&
        getClearColour() const;
