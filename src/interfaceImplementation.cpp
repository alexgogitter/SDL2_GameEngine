...
Interface::Interface(SDL_Window* applicationWindow, SDL_GLContext applicationContext)
    : window(applicationWindow), glContext(applicationContext)
{
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
...
bool Interface::initialiseOpenGLRenderer()
{
    ...
    ImGui::GetIO().Fonts->SetTexID(
        static_cast<ImTextureID>(static_cast<intptr_t>(fontTexture))
    );
...
void Interface::renderDrawData(ImDrawData* drawData)
{
    ...
    for (int listIndex = 0; listIndex < drawData->CmdListsCount; ++listIndex)
    {
        ...
        for (int commandIndex = 0; commandIndex < commandList->CmdBuffer.Size; ++commandIndex)
        {
            ...
            glScissor(
                static_cast<int>(clipMinimum.x),
                static_cast<int>(framebufferHeight - clipMaximum.y),
                static_cast<int>(clipMaximum.x - clipMinimum.x),
                static_cast<int>(clipMaximum.y - clipMinimum.y)
            );

            glActiveTexture(GL_TEXTURE0);
            glBindTexture(
                GL_TEXTURE_2D,
                static_cast<unsigned int>(static_cast<intptr_t>(command->GetTexID()))
            );
...
void Interface::addDrawCallback(std::function<void()> callback)
{
    drawCallbacks.push_back(callback);
}

void Interface::update(SDL_Event& event)
{
