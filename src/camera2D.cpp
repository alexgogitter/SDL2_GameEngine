...
{
    zoom = std::max(value, 0.01f);
}

glm::vec2 Camera2D::screenToWorld(const glm::vec2& screenPosition) const
{
    const glm::vec2 normalisedDevicePosition = {
        (2.0f * screenPosition.x / viewportSize.x) - 1.0f,
        1.0f - (2.0f * screenPosition.y / viewportSize.y)
    };

    const glm::vec4 worldPosition = glm::inverse(getViewProjectionMatrix()) *
        glm::vec4(normalisedDevicePosition, 0.0f, 1.0f);
