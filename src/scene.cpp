...
{
    if (
        requestedId != InvalidObjectId &&
        findObject(requestedId) != nullptr
    )
    {
        return nullptr;
    }
...
Object* Scene::createChildObject(
    ObjectId parentId,
    const std::string& name,
    ObjectId requestedId)
{
...
bool Scene::destroyObject(ObjectId objectId)
{
    ...
    for (auto iterator = impl->rootObjects.begin();
         iterator != impl->rootObjects.end();
         ++iterator)
    {
...
)
{
    ...
    Object* newParent =
        newParentId != InvalidObjectId
            ? findObject(newParentId)
            : nullptr;
    ...
    if (
        newParent != nullptr &&
        object->findInHierarchy(newParentId) != nullptr
    )
    {
        return false;
    }

    Object* previousParent =
    ...
    if (keepWorldTransform)
    {
        ...
        if (newParent != nullptr)
        {
            ...
            if (
                std::abs(glm::determinant(parentWorld)) <=
                0.000001f
            )
            {
                return false;
    ...
    {
        ownership.reset(
            previousParent->detachChild(object)
        );
    }
    else
    {
    ...
    if (newParent != nullptr)
    {
        ...
        )
        {
            ...
            {
                if (
                    previousParent->add_Child_Object(
                        ownership.get()
                    ) == 0
                )
                {
