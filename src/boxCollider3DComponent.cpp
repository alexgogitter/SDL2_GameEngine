...
namespace
{
    ...
    physx::PxQuat ToPhysX(const glm::quat& value)
    {
        return {
            value.x,
            value.y,
...
struct BoxCollider3DComponent::Impl
{
    ...
    physx::PxRigidStatic* ownedStaticActor =
        nullptr;
      ...
      impl(std::make_unique<Impl>())
{
    ...
    else
    {
        ...
        impl->ownedStaticActor =
            physics->createRigidStatic(
                pose
            );
...
bool BoxCollider3DComponent::isValid() const
{
    return
        impl != nullptr &&
        impl->shape != nullptr &&
        impl->shape->getActor() != nullptr;
}
...
bool BoxCollider3DComponent::GetProperty(
    std::size_t index,
...
)
{
    ...
    else if (std::strcmp(key, "center") == 0)
    {
        ...
        std::strcmp(key, "staticFriction") == 0 ||
        std::strcmp(key, "dynamicFriction") == 0 ||
        std::strcmp(key, "restitution") == 0
    )
    {
        staticFriction =
            glm::clamp(staticFriction, 0.0f, 1.0f);

        dynamicFriction =
            glm::clamp(dynamicFriction, 0.0f, 1.0f);
...
updateGeometry()
{
    ...
    impl->shape->setLocalPose(
        physx::PxTransform(
            ToPhysX(scaledCenter)
        )
    );
...
updateMaterial()
{
    ...
    impl->material->setStaticFriction(
        staticFriction
    );

    impl->material->setDynamicFriction(
        dynamicFriction
    );

    impl->material->setRestitution(
        restitution
    );
}
...
void BoxCollider3DComponent::OnEnable()
{
    updateShapeFlags();
}
