...
namespace
{
        ...
        const std::string& referencedPath)
    {
        ...
        const bool isWindowsAbsolute =
            referencedPath.size() > 2 &&
            referencedPath[1] == ':' &&
            (referencedPath[2] == '\\' || referencedPath[2] == '/');
        const bool isRootAbsolute =
            referencedPath.front() == '\\' || referencedPath.front() == '/';
    ...
    glm::vec4 toNormalizedUvRect(
        const glm::vec4& pixelRect,
        float atlasWidth,
        float atlasHeight)
    {
        return {
            ...
            pixelRect.z / atlasWidth,
            pixelRect.w / atlasHeight
        };
    }

    bool parseSpriteName(
        const std::string& name,
        std::string& prefix,
        int& frameNumber,
        int& angle)
    ...
    void assignParsedSprite(
        PartialFrame& frame,
        const ParsedSprite& sprite,
...
AnimationControllerComponent::AnimationControllerComponent(
    ...
    SpriteRendererComponent& targetSprite)
    : Component("AnimationController2D", parentObject),
      resources(&resourceManager),
      sprite(&targetSprite)
{
}

bool AnimationControllerComponent::loadTexturePackerAtlas(
    const std::string& xmlPath,
    const std::string& clipBaseName,
    float fps,
    bool loop)
{
    ...
    const TextureHandle srgbAtlas =
        resources->loadTexture(imagePath.c_str(), TextureColourSpace::SRGB);
    const TextureHandle linearAtlas =
        resources->loadTexture(imagePath.c_str(), TextureColourSpace::Linear);
    ...
    for (auto iterator = std::sregex_iterator(xml.begin(), xml.end(), spritePattern);
         iterator != std::sregex_iterator();
         ++iterator)
    {
        ...
        if ((hasAttribute(attributes, "r") && attributes.at("r") == "y") ||
            hasAttribute(attributes, "vertices") ||
            hasAttribute(attributes, "triangles"))
        {
            std::fprintf(
                stderr,
                "ERROR: TexturePacker sprite %s uses rotation/polygon packing; v1 supports rectangles only.\n",
                ...
                parsed.angle))
        {
            std::fprintf(
                stderr,
                "WARNING: Ignoring TexturePacker sprite with unsupported name: %s\n",
                nameAttribute->second.c_str()
            );
            continue;
        }
        ...
        parsed.pixelRect = {
            static_cast<float>(readIntAttribute(attributes, "x")),
            static_cast<float>(readIntAttribute(attributes, "y")),
            static_cast<float>(readIntAttribute(attributes, "w")),
            static_cast<float>(readIntAttribute(attributes, "h"))
        };
        ...
        const glm::vec4 uvRect = toNormalizedUvRect(
            parsed.pixelRect,
    ...
    for (auto& anglePair : framesByAngle)
    {
        ...
        for (auto& framePair : anglePair.second)
        {
            ...
            if (!partial.hasAlbedo)
            {
                std::fprintf(
                    stderr,
                    "WARNING: Skipping animation frame %d for %s because Image map is missing.\n",
                    framePair.first,
                    clip.name.c_str()
                );
                continue;
...
bool AnimationControllerComponent::setDefaultState(const std::string& stateName)
{
    return play(stateName, true);
}

bool AnimationControllerComponent::addTransition(
...
bool AnimationControllerComponent::addAnyTransition(
    const std::string& eventName,
    const std::string& toState)
{
...
bool AnimationControllerComponent::play(const std::string& stateName, bool restart)
{
    ...
    {
        currentFrameIndex = std::min(
            currentFrameIndex,
            found->second.frames.size() - 1
        );
    }
    applyCurrentFrame();
    return true;
...
const AnimationControllerComponent::AnimationClip*
AnimationControllerComponent::getCurrentClip() const
{
    const auto found = clips.find(currentState);
    return found != clips.end() ? &found->second : nullptr;
}
